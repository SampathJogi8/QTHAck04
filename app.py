...
v6 — All graph/module bugs fixed + Apple-grade UI
"""
...
.dsp-header-left h1 {
    color: #f0f4f8; margin: 0 0 5px 0; font-size: 20px;
    font-weight: 700; letter-spacing: -0.03em;
}
.dsp-header-left p {
    color: #5a6882; margin: 0; font-size: 12px; line-height: 1.5;
}
.dsp-header-badge {
    background: rgba(0,210,255,0.06);
    border: 1px solid rgba(0,210,255,0.18);
    border-radius: 10px; padding: 8px 16px;
    font-size: 11px; color: #7ab8cc;
...
.mcard .val {
    ...
    letter-spacing: -0.02em; margin-top: 7px; display: block; color: #e2e8f0;
}
.mcard .lbl {
    font-size: 9.5px; color: #3d4d5e;
...
.badge-warn {
    display: inline-flex; align-items: center; gap: 6px;
    background: rgba(251,113,60,0.12); color: #fb713c;
    padding: 4px 12px; border-radius: 20px; font-size: 11.5px; font-weight: 600;
...
.audio-banner {
    ...
    display: flex; align-items: flex-start; gap: 12px; flex-wrap: wrap; line-height: 1.6;
}
...
.section-header h3 {
    color: #c8d3e0; font-size: 13.5px; font-weight: 600; margin: 0;
}
.section-num {
    background: rgba(0,210,255,0.08); border: 1px solid rgba(0,210,255,0.2);
    color: #00d2ff; width: 24px; height: 24px; border-radius: 7px;
...
.sidebar-section-title {
    font-size: 10px; font-weight: 700; color: #3a4554;
    text-transform: uppercase; letter-spacing: 0.13em;
    padding: 14px 0 6px 0; border-bottom: 1px solid rgba(255,255,255,0.05);
    margin-bottom: 12px; margin-top: 4px;
}

.stTabs [data-baseweb="tab-list"] {
    background: #0a0d14; border-bottom: 1px solid rgba(255,255,255,0.06);
    gap: 0; padding: 0;
}
.stTabs [data-baseweb="tab"] {
    background: transparent; color: #4a5568 !important; border-radius: 0;
    border-bottom: 2px solid transparent; padding: 11px 20px;
    font-size: 12.5px; font-weight: 500; transition: color 0.15s, border-color 0.15s;
}
.stTabs [data-baseweb="tab"]:hover {
    color: #8a97aa !important; background: rgba(255,255,255,0.02);
}
.stTabs [aria-selected="true"] {
    color: #00d2ff !important; border-bottom-color: #00d2ff !important;
    background: transparent;
}

.streamlit-expanderHeader {
    background: #0c1018 !important; border: 1px solid rgba(255,255,255,0.07) !important;
    border-radius: 10px !important; color: #8a97aa !important; font-size: 12.5px !important;
}

.stAlert { background: #0c1018 !important; border-radius: 10px !important; font-size: 12.5px !important; }
...
.info-box {
    ...
    color: #3d4d5e; font-size: 13px; margin: 24px 0;
}
...
.stMarkdown table {
    background: #0c1018 !important; border-radius: 10px; overflow: hidden;
    border: 1px solid rgba(255,255,255,0.07) !important; font-size: 12.5px;
}
.stMarkdown th {
    background: #111827 !important; color: #8a97aa !important;
    font-weight: 600 !important; border-color: rgba(255,255,255,0.06) !important;
}
.stMarkdown td {
    color: #c8d3e0 !important; border-color: rgba(255,255,255,0.05) !important;
...
def next_pow2(n: int) -> int:
    """Return smallest power of two ≥ max(n, 1)."""
    n = max(int(n), 1)
    p = 1
    while p < n:
        p <<= 1
...
def gen_signal(time_arr: np.ndarray, f: float, a: float, wtype: str, dur: float) -> np.ndarray:
    """Generate a synthetic waveform over time_arr."""
    f = max(float(f), 1e-9)
    a = float(a)
    if wtype == "Sine":
    ...
    if wtype == "Triangle":
        return a * (2.0 / np.pi) * np.arcsin(
            np.clip(np.sin(2 * np.pi * f * time_arr), -1.0, 1.0))
    if wtype == "Sawtooth":
...
def compute_fft(sig: np.ndarray, display_fs: float):
    """
    Compute FFT with correct frequency axis.
    display_fs = actual sample rate of sig array = len(sig)/duration_eff
    Returns (freqs_hz, mags, power_db, n_fft)
    """
    display_fs = max(float(display_fs), 1.0)
    ...
    freqs = np.fft.rfftfreq(n, d=1.0 / display_fs)
...
def load_wav(file_obj) -> tuple:
    """Load WAV → normalised float64 mono array. Returns (audio, rate)."""
...
def apply_theme(fig, height: int = 420, title: str = "",
                xtitle: str = "Time (s)", ytitle: str = "Amplitude"):
...
def mcard(label: str, value: str, accent: str = None):
    c = accent or COLORS["cyan"]
    st.markdown(
        f'<div class="mcard"><div class="lbl">{label}</div>'
        f'<span class="val" style="color:{c}">{value}</span></div>',
        unsafe_allow_html=True,
    )
...
def section_header(num: str, title: str):
    st.markdown(
        f'<div class="section-header"><div class="section-num">{num}</div>'
        f'<h3>{title}</h3></div>',
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    if audio_loaded:
        ...
        signal_type = st.selectbox(
            "Waveform *(inactive)*", ["Sine", "Square", "Triangle", "Sawtooth", "Chirp"],
            disabled=True)
        ...
        fs_raw   = st.slider("Sampling Rate (Hz)", 2, 200, 40,
                             help="Samples per second taken from the audio signal.")
    else:
        signal_type = st.selectbox(
        ...
        fs_raw  = st.slider("Sampling Rate (Hz)", 2, 200, 40,
                            help="Set below 2× frequency to trigger aliasing.")
    ...
    noise_level = st.slider("Noise Level", 0.0, 2.0, 0.0, 0.1,
                            help="Std dev of additive Gaussian noise.")
    filter_win  = st.slider("Filter Window (samples)", 2, 80, 10,
                            help="Moving-average FIR filter length.")
...
# KEY: N=4000 display points cover duration_eff seconds.
#      The *effective* display sample rate is display_fs = N / duration_eff.
#      The user-chosen fs (fs_raw) controls the SAMPLED signal only.
#      All FFT frequency axes must use display_fs, not fs.
# ══════════════════════════════════════════════════════════════════════════════
fs = max(int(fs_raw), 2)   # user-chosen sampling rate for stem plot
N  = 4000                  # number of display points
...
# Effective display sample rate — used for ALL FFT frequency axes on the N-point arrays
display_fs = N / duration_eff   # e.g. 4000/2 = 2000 Hz

nyquist  = 2 * freq_eff
# Alias frequency: fold freq_eff into [0, fs/2]
...
if "audio_data" in st.session_state:
    ...
    st.markdown(
        f'<div class="audio-banner"><div>🎵</div><div>'
        ...
        f'<span class="warn-note">⚠ Waveform / Frequency / Amplitude / Duration sliders '
        f'overridden. Only Sampling Rate, Noise and Filter controls are active.</span>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
...
(tab_scope, tab_fft, tab_phase,
 tab_filter, tab_water, tab_liss, tab_audio) = st.tabs([
    "  Oscilloscope  ", "  FFT Spectrum  ", "  Phase Space  ",
    "  Filter Lab  ", "  Waterfall  ", "  Lissajous  ", "  Audio Analyzer  ",
])
...
with tab_scope:
    ...
    with col1:
        ...
        if noise_level > 0:
            fig.add_trace(go.Scatter(
                ...
                line=dict(color=COLORS["red"], width=1.0), opacity=0.55,
            ))
    ...
    with col2:
        ...
        fig2.add_trace(go.Scatter(
            ...
            line=dict(color=COLORS["green"], width=1.2, dash="dot"), opacity=0.3,
        ))
        # Stem lines for sampled points
        stem_x, stem_y = [], []
    ...
    # Alias signal uses same waveform type as original
    alias_sig = gen_signal(t, float(alias_f) if alias_f > 0 else float(freq_eff),
                           float(amp), signal_type, duration_eff)
    fig3 = go.Figure()
    ...
    apply_theme(fig3, height=420, title=f"alias = {alias_f:.2f} Hz   [{note}]")
    st.plotly_chart(fig3, use_container_width=True)

    if is_alias:
        st.error(
            f"**Aliasing detected!**  {freq_eff} Hz requires Fs ≥ {nyquist} Hz. "
            f"At Fs = {fs} Hz, the signal appears as **{alias_f:.2f} Hz**.", icon="⚠️",
        )
    else:
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 2 — FFT SPECTRUM
# BUG FIX: must use display_fs (N/duration_eff) not user fs for frequency axis.
# The 'noisy' array has N=4000 points over duration_eff seconds,
# so its actual sample rate is display_fs, not the user-chosen fs.
# ══════════════════════════════════════════════════════════════════════════════
with tab_fft:
    # FIXED: pass display_fs so frequency axis is correct
    freqs_hz, mags, power_db, n_fft = compute_fft(noisy, display_fs)
    ...
    # Downsample for bar chart display (keep ≤ 800 bars for performance)
    # Show only the meaningful low-frequency range up to Nyquist of user fs
    max_display_freq = min(display_fs / 2.0, freq_eff * 10 + 100)
    mask_disp = freqs_hz <= max_display_freq
    fhz_disp  = freqs_hz[mask_disp]
    mags_disp = mags[mask_disp]
    pdb_disp  = power_db[mask_disp]

    # Further downsample if too many bins
    ds_factor = max(1, len(fhz_disp) // 800)
    fhz_ds  = fhz_disp[::ds_factor]
    mags_ds = mags_disp[::ds_factor]
    pdb_ds  = pdb_disp[::ds_factor]

    bar_colors = [
        f"hsl({int(200 + i / max(len(fhz_ds) - 1, 1) * 100)},65%,52%)"
        for i in range(len(fhz_ds))
    ]

    fig_mag = go.Figure()
    fig_mag.add_trace(go.Bar(
        x=fhz_ds, y=mags_ds, name="Magnitude",
        marker=dict(color=bar_colors, line=dict(width=0)),
        showlegend=False,
    ))
    # Safe vline: guard with explicit boolean check
    if len(fhz_disp) > 0 and 0 < dom_freq <= float(fhz_disp[-1]):
        fig_mag.add_vline(
    ...
    if is_alias and len(fhz_disp) > 0 and (0 < alias_f <= float(fhz_disp[-1])):
        fig_mag.add_vline(
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 3 — PHASE SPACE
# BUG FIX: autocorrelation slicing was unsafe for ll near N.
# ══════════════════════════════════════════════════════════════════════════════
with tab_phase:
    lag = st.slider("Lag k (samples)", 1, min(50, N // 4), 5, key="lag_slider")
    lag = min(lag, len(noisy) - 2)   # ensure at least 2 points remain
    x_ph = noisy[:-lag]
    y_ph = noisy[lag:]

    max_lag = min(200, len(signal) // 4)   # capped for speed
    mean_s  = float(signal.mean())
    var_s   = float(np.var(signal))
    lags_arr = np.arange(max(max_lag, 2))

    if var_s > 1e-12:
        # FIXED: use min slice length to avoid shape mismatch
        auto = np.array([
            float(np.mean(
                (signal[:N - ll] - mean_s) * (signal[ll:N] - mean_s)
            )) / var_s
            for ll in lags_arr
        ])
    else:
    ...
    with col2:
        ...
        fig_ac.add_hline(y=0, line_dash="dot", line_color="rgba(255,255,255,0.1)")
...
with tab_filter:
    ...
    # Cutoff: in terms of display sample rate (the rate of the N-point arrays)
    cutoff   = round(display_fs / (2.0 * max(int(filter_win), 1)), 2)
    ...
    with col_n:
        ...
        apply_theme(fig_noisy, height=340, title=f"Noise level σ = {noise_level:.1f}")
    ...
    fig_cmp.add_trace(go.Scatter(
        x=t, y=signal, mode="lines", name="Original (clean)",
        line=dict(color=COLORS["green"], width=1.4, dash="dot"), opacity=0.55,
    ))
    if noise_level > 0:
        fig_cmp.add_trace(go.Scatter(
            x=t, y=noisy, mode="lines",
            name=f"Noisy (σ = {noise_level:.1f})",
            line=dict(color=COLORS["red"], width=1.0), opacity=0.6,
        ))
    fig_cmp.add_trace(go.Scatter(
        x=t, y=filtered, mode="lines",
        name=f"Filtered (M = {filter_win})",
    ...
    if noise_level == 0:
        st.info(
            "**Noise level is 0** — increase Noise Level slider to see the "
            "Noisy vs Filtered comparison.", icon="ℹ️",
        )
    st.plotly_chart(fig_cmp, use_container_width=True)

    section_header("05", "Filter Frequency Response |H(f)|")
    omega = np.linspace(0.0, np.pi, 2048)
    eps   = 1e-9
    M_f   = float(max(int(filter_win), 1))
    H     = np.clip(
        np.abs(np.sin(M_f * omega / 2.0 + eps) / (M_f * np.sin(omega / 2.0 + eps) + eps)),
        0.0, 1.0
    )
    # Frequency axis in Hz, mapped to display_fs
    f_ax_hr = omega / np.pi * (display_fs / 2.0)
    # Show only up to a sensible max for the current display rate
    f_max_hr = min(display_fs / 2.0, 500.0)
    hr_mask  = f_ax_hr <= f_max_hr
    fig_hr   = go.Figure()
    fig_hr.add_trace(go.Scatter(
        x=f_ax_hr[hr_mask], y=H[hr_mask], mode="lines", name="|H(f)|",
        line=dict(color=COLORS["orange"], width=2.2),
        fill="tozeroy", fillcolor="rgba(251,146,60,0.07)",
    ))
    fig_hr.add_hline(
        y=0.707, line_dash="dash", line_color="rgba(255,255,255,0.2)",
        annotation_text="−3 dB (0.707)",
        annotation_font=dict(color="#8a97aa", size=11),
        annotation_position="bottom right",
    )
    apply_theme(fig_hr, height=360,
                title=f"FIR Moving-Average Frequency Response (M={filter_win})",
                xtitle="Frequency (Hz)", ytitle="|H(f)|")
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 5 — WATERFALL
# BUG FIX: rfftfreq must use display_fs (N/duration_eff), not user fs.
# The frame data comes from 'noisy' which is an N-point array at display_fs.
# ══════════════════════════════════════════════════════════════════════════════
with tab_water:
    ...
    for i in range(n_frames):
        ...
        padded[:frame_len] = chunk * win_h
        wfall.append(np.abs(np.fft.rfft(padded)) / max(n_fft_w / 2.0, 1.0))
        t_labels.append(round(start / display_fs, 4))   # real time in seconds

    if not wfall:
        st.warning("Signal too short for waterfall. Increase Duration.")
    else:
        wfall_arr = np.array(wfall)
        # FIXED: use display_fs for correct frequency axis
        f_ax_w = np.fft.rfftfreq(n_fft_w, d=1.0 / display_fs)
        z_db   = np.clip(20.0 * np.log10(wfall_arr + 1e-12), -80.0, 0.0)

        # Limit frequency axis to a sensible range (up to 2× signal freq or 500 Hz)
        f_limit = min(display_fs / 2.0, max(freq_eff * 4, 50.0))
        f_mask  = f_ax_w <= f_limit
        f_show  = f_ax_w[f_mask]
        z_show  = z_db[:, f_mask]

        fig_wf = go.Figure(go.Heatmap(
            z=z_show.T, x=t_labels, y=f_show,
        ...
        freq_res_w = round(display_fs / n_fft_w, 3)
        st.caption(
            f"Hanning STFT · {len(wfall)} frames · FFT size {n_fft_w} · "
            f"{freq_res_w} Hz/bin · Display Fs = {display_fs:.0f} Hz · "
            f"Tip: select Chirp waveform for a diagonal sweep"
        )


# ══════════════════════════════════════════════════════════════════════════════
# TAB 6 — LISSAJOUS
# BUG FIX: t_liss needs enough points to resolve both frequencies without aliasing.
# ══════════════════════════════════════════════════════════════════════════════
with tab_liss:
...
Non-integer ratios → quasi-periodic open paths.
    ...
    with col_plot:
        ...
        # Show exactly lcm_f periods (capped), at a sample rate that resolves both frequencies
        lcm_f     = (fe_int * f2_int) // max(gcd_v, 1)
        n_periods = min(max(gcd_v, 1) * 6, 20)  # show enough periods for full pattern
        t_end     = float(n_periods) / float(min(fe_int, f2_int))
        # Sample at 500× the highest frequency to avoid aliased-looking curves
        n_pts     = min(int(t_end * max(fe_int, f2_int) * 500), 40000)
        n_pts     = max(n_pts, 3000)
        t_liss    = np.linspace(0.0, t_end, n_pts)
...
# BUG FIX: all FFT/spectrogram frequency axes must use actual audio rate.
# ══════════════════════════════════════════════════════════════════════════════
with tab_audio:
    ...
    if "audio_data" not in st.session_state:
        ...
        # FIXED: compute_fft uses actual audio rate for correct frequency axis
        fa, spec_mags, _, n_fft_a = compute_fft(clip_fft, float(rate))

        fig_spec = go.Figure()
        fig_spec.add_trace(go.Scatter(
            x=fa, y=spec_mags, mode="lines",
        ...
        # Mark dominant peak
        if len(spec_mags) > 0:
            peak_f = float(fa[int(np.argmax(spec_mags))])
            fig_spec.add_vline(
                x=peak_f, line_dash="dash", line_color=COLORS["orange"],
                annotation_text=f"peak: {peak_f:.1f} Hz",
                annotation_font=dict(color=COLORS["orange"], size=11),
                annotation_position="top right",
            )
        ...
        if n_fr < 2:
            ...
            win_sg   = np.hanning(frame_sz)   # window matches frame_sz exactly
            ...
            # FIXED: use actual audio rate for correct frequency axis
            ...
            st.caption(
                ...
                f"FFT {n_fft_sg} · {n_fr} frames · Fs = {rate} Hz"
            )
